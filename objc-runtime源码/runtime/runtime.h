...
struct objc_class {
    /**
        objc_object（实例对象）中isa指针指向的类结构成为class（也就是该对象所属的类）其中存放着普通成员变量与动态方法（-号开头）；此处isa指针指向的类结构称为metaclass，其中存放着static类型的成员变量与static类型的方法（+号方法）
     */
    Class isa  OBJC_ISA_AVAILABILITY;   // metaclass 元类
    

#if !__OBJC2__
    // 父类/超类
    // 指向该类的父类指针，如果该类是根类（如NSObject或者NSProxy），那么super_class就为nil
    /*
    ...
    // 类的版本信息，默认为0，可以通过runtime函数class_setVesion或者class_getVersion进行修改、读取
    long version                                             OBJC2_UNAVAILABLE;
    // 类信息，供运行时期使用的一些位标识，如CLS_CLASS（0x1L）表示该类为普通class，其中包含实例方法变量；
    ...
    // 方法地址列表，与info的一些标志位有关，如CLS_CLASS(0x1L),则存储实例方法，
    // 如CLS_META（0x2L） 表示该类为metaclass 其中包含类方法
...
// 属性的attribute
// 其实是对属性的详细描述，包括属性名称、属性编码类型、原子类型/非原子类型等
typedef struct {
    // 名称
...
#endif
struct objc_cache {
    // 被指定分配cache buckets的总数。在查找方法中，Runtime使用这个字段确定数组的所以位置
    unsigned int mask /* total = mask + 1 */                 OBJC2_UNAVAILABLE;
    // 实际占用cache buckets的总数
    unsigned int occupied                                    OBJC2_UNAVAILABLE;
    // 指定Method数据结构指针的数组，这个数组可能包含不超过mask + 1 个元素。需要注意的是，指针可能是NULL，这个表示缓存的bucket没有被占用。另外被占用的bucket可能是不连续的。这个数组可能会随着时间而增长。
